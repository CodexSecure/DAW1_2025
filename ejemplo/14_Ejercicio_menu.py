# ==============================================
# EJERCICIOS AVANZADOS DE BUCLES (INTERACTIVOS)
# Solo 3 ejercicios difíciles
# ==============================================

def ejercicio1():
    # EJERCICIO 1: Suma de números pares de una lista
    print("\n=== EJERCICIO 1: Suma de números pares de una lista ===")

    # lista = [int(x) for x in input("Introduce números separados por espacios: ").split()]

    # Pedimos la entrada al usuario
    entrada = input("Introduce números separados por espacios: ").split()

    # Convertimos cada elemento a entero sin usar comprensión de listas
    lista = []
    for elemento in entrada:
        numero = int(elemento)
        lista.append(numero)

    # Calculamos la suma de los pares
    suma = 0
    for num in lista:
        if num % 2 == 0:
            suma += num

    print("La suma de los números pares es:", suma)

def ejercicio2():
    # EJERCICIO 2: Adivinar un número con intentos limitados
    print("\n=== EJERCICIO 2: Adivina el número (1-50) ===")
    import random
    secreto = random.randint(1, 50)
    print(f"El secreto es : {secreto}")
    intentos = 0
    while True:
        adivina = int(input("Introduce un número: "))
        intentos += 1
        if adivina < secreto:
            print("Demasiado bajo")
        elif adivina > secreto:
            print("Demasiado alto")
        else:
            print(f"¡Correcto! Número de intentos: {intentos}")
            break

def ejercicio3():
    # EJERCICIO 3: Calcular la media de números positivos hasta introducir negativo
    print("\n=== EJERCICIO 3: Media de números positivos hasta negativo ===")
    suma = 0
    contador = 0
    while True:
        num = int(input("Introduce un número (negativo para salir): "))
        if num < 0:
            break
        suma += num
        contador += 1
    if contador == 0:
        print("No se introdujeron números positivos.")
    else:
        print("Media de los números positivos:", suma / contador)

# ==============================================
# MENÚ INTERACTIVO
# ==============================================
while True:
    print("\n--- MENÚ DE EJERCICIOS AVANZADOS ---")
    print("1: Suma de números pares de una lista")
    print("2: Adivinar un número")
    print("3: Media de números positivos")
    print("0: Salir")
    opcion = input("Elige un ejercicio (0-3): ")

    if opcion == "1":
        ejercicio1()
    elif opcion == "2":
        ejercicio2()
    elif opcion == "3":
        ejercicio3()
    elif opcion == "0":
        print("¡Hasta luego!")
        break
    else:
        print("Opción no válida. Intenta de nuevo.")
